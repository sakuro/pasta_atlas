...
upload-preparing = { $prepared } / { $total } ファイルの準備中...
...
upload-verifying = { $progress } / { $total } ファイルを検証中...
upload-error-verify-http = 検証に失敗しました（HTTP { $status }）。
upload-error-verify-network = 検証中にネットワークエラーが発生しました。接続を確認して再度お試しください。
...
map-control-zoom-in = ズームイン
map-control-zoom-out = ズームアウト
map-info-button =
    .title = マップ情報
