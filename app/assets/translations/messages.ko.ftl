...
upload-preparing = { $prepared } / { $total } 파일 준비 중...
upload-progress = { $progress } / { $total } 파일 업로드 중...
upload-complete = 업로드 완료!
upload-view-map = 맵 보기
upload-close = 닫기
upload-dismiss = 닫기
upload-error-too-many-images = 이미지가 너무 많습니다({ $count }개). 최대 { $max }개입니다.
...
upload-verifying = { $progress } / { $total } 파일 검증 중...
upload-error-verify-http = 검증에 실패했습니다 (HTTP { $status }).
upload-error-verify-network = 검증 중 네트워크 오류가 발생했습니다. 연결을 확인하고 다시 시도해 주세요.
...
map-control-zoom-in = 확대
map-control-zoom-out = 축소
map-info-button =
    .title = 맵 정보
