...
upload-instructions-folder = mapshot.json이 포함된 폴더를 선택해 주세요.
upload-instructions-folder-path = { -mapshot } 기본 폴더:
...
how-to-upload-achievement-warning = 도전 과제에 영향을 주지 않으려면 { -mapshot }이 설치된 상태에서 게임을 저장하지 마세요. 캡처 후 { -mapshot }을 제거하세요.
