...
edit-display-name-help = 最多 30 个字符，留空则显示用户名。
edit-save-profile = 保存个人资料
edit-timezone-label = 时区
edit-timezone-use-browser = （使用浏览器设置）
edit-locale = 语言
...
upload-instructions-folder-path = { -mapshot } 的输出文件夹（Steam 版）：
...
upload-copy-path-linux =
    .title = 复制路径 — 在文件夹对话框中按 Ctrl+L 粘贴
upload-instructions-folder-subfolder = 接着选择您的地图与世代的子文件夹。例如：
...
upload-preparing = 正在准备 { $prepared } / { $total } 个文件...
upload-progress = 正在上传第 { $progress } / { $total } 个文件...
upload-complete = 上传完成！
upload-view-map = 查看地图
upload-close = 关闭
upload-dismiss = 关闭
upload-error-too-many-images = 图片过多（{ $count } 张）。上限为 { $max } 张。
...
upload-verifying = 正在验证 { $progress } / { $total } 个文件...
upload-error-verify-http = 验证失败（HTTP { $status }）。
upload-error-verify-network = 验证时发生网络错误。请检查您的连接后重试。
