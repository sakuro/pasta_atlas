...
registration-username = 使用者名稱
    .help = 1–39 個字元，可使用英文字母、數字、連字號和底線，開頭與結尾必須為英文字母或數字。
registration-submit = 建立帳號

## Registration errors
error-username-empty = 請輸入使用者名稱。
error-username-too-long = 使用者名稱不得超過 39 個字元。
...
upload-instructions-folder = 請選取包含 mapshot.json 的資料夾。
upload-instructions-folder-path = { -mapshot } 的典型資料夾路徑：
...
upload-instructions-guest = 訪客帳號的上傳將於約一週後刪除。
...
how-to-upload-step1-body = 從 { -factorio } 遊戲內的 MOD 選單安裝 { -mapshot }，然後重新啟動遊戲。
...
how-to-upload-step3-select-folder = 點擊「選取資料夾」並選取包含 <code data-l10n-name="filename">mapshot.json</code> 的子資料夾（請選取資料夾，而非檔案本身）。
how-to-upload-step3-confirm = 確認地圖標題與詳細資訊後，點擊「開始上傳」。
