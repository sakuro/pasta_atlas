...
A comment is anchored to a specific point identified by three values:
...
3. Current zoom = `zoom_max` of the active surface (comments are created at and visible only at maximum zoom).
...
| Jump to location | From list entry → fly to (world_x, world_y) on the correct surface at zoom_max |
...
Comment creation is restricted to `zoom_max`. At any other zoom level the UI blocks creation and prompts the user to zoom in.
...
- At `zoom_max`: button is active and clickable.
- Below `zoom_max`: button is visually disabled; hovering shows a tooltip such as "Zoom in to maximum to place a comment".
...
- At `zoom_max`: opens the comment form as normal.
- Below `zoom_max`: ignored (no form opens, no visual feedback required since the disabled button already communicates the constraint).

Both methods record `(world_x, world_y, surface_index)` from the click event and current map state.
...
- Compare `Math.round(map.getZoom())` against `surface.zoom_max` to determine whether creation is allowed.
