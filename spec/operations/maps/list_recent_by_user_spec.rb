...
RSpec.describe PastaAtlas::Operations::Maps::ListRecentByUser do
  ...
  let(:operation) { PastaAtlas::Operations::Maps::ListRecentByUser.new(map_repo:, generation_repo:, settings:) }

  let(:user_info) { PastaAtlas::Values::UserInfo[name: "alice", display_name: "Alice", avatar_url: nil] }
  ...
  describe "#call" do
    context "when the user has maps" do
      ...
      it "returns success with map infos" do
        result = operation.call(user_id: 1, user_info:)
        ...
        expect(map_info.user_info).to eq(user_info)
