...
RSpec.describe PastaAtlas::Views::Maps::Index do
  ...
  def render(maps:, users_by_id:, profiles_by_user_id:, page:, per_page:, total:, thumbnail_urls_by_map_ulid: {})
    view.call(layout: false, maps:, users_by_id:, profiles_by_user_id:, thumbnail_urls_by_map_ulid:, page:, per_page:, total:).to_s
  end
  ...
  it "renders the user display name with a profile link" do
    ...
    expect(html).to include(%(<a href="/@alice/profile">Alice</a>))
  end
