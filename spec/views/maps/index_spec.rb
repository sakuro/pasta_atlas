...
RSpec.describe PastaAtlas::Views::Maps::Index do
  ...
  it "renders a link for each map" do
    html = render(maps: [map_with_profile, map_without_profile], user_profiles_by_id: {}, page: 1, per_page: 20, total: 2)

    expect(html).to include(
      %(<a href="/maps/01MAP1">First Map</a>),
      %(<a href="/maps/01MAP2">Second Map</a>)
    )
  end
  ...
  it "omits the user profile name when not available" do
    html = render(maps: [map_with_profile], user_profiles_by_id: {}, page: 1, per_page: 20, total: 1)

    expect(html).not_to include("alice")
  end
  ...
  it "renders a next link when more pages remain" do
    html = render(maps: [map_with_profile], user_profiles_by_id: {}, page: 1, per_page: 1, total: 2)
  ...
  it "renders a previous link on pages after the first" do
    html = render(maps: [map_without_profile], user_profiles_by_id: {}, page: 2, per_page: 1, total: 2)
